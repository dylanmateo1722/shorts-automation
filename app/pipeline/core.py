...
def ejecutar_run(
    ...
    *,
    ...
    runner = StageRunner(workspace, manifest, settings)
    try:
        for etapa in construir_pipeline():
...
def validar_run(run_id: UUID, settings: Settings | None = None) -> tuple[bool, list[str]]:
